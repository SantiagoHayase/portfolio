import React from "react";
import Image from "next/image";
import videogameApiImg from "../../public/assets/Videogamesapi.jpg";
import Link from "next/link";

const videogamesapi = () => {
  return (
    <div>en construccion</div>
    // <div id="projects" className="w-full pt-10">
    //   <div className="max-w-[1240px] mx-auto px-2 py-16">
    //     <div className="grid lg:grid-cols-5 gap-8">
    //       {/* left */}
    //       <div className="col-span-3 lg:col-span-2 w-full h-full">
    //         <Image
    //           src={videogameApiImg}
    //           alt={"/"}
    //           height={450}
    //           className="rounded-xl"
    //         />
    //       </div>
    //       {/* right */}
    //       <div className="col-span-3 w-full h-auto lg:p-4">
    //         <p className="text-xl tracking-widest uppercase text-[#5651e5]">
    //           Videogames' api
    //         </p>
    //         <div className="flex">
    //           <p className="m-3 shadow-xl rounded">JavaScript</p>
    //           <p className="m-3 shadow-xl rounded">React.js</p>
    //           <p className="m-3 shadow-xl rounded">Redux</p>
    //           {/* <p className="m-3 shadow-xl rounded">TypeScript</p> */}
    //         </div>
    //         <p>
    //           Lorem ipsum dolor sit amet consectetur, adipisicing asd asdas
    //           dLorem ipsum dolor sit amet consectetur, adipisicing asd asdas
    //           dLorem ipsum dolor sit amet consectetur, adipisicing asd asdas
    //           dLorem ipsum dolor sit amet consectetur, adipisicing asd asdas
    //           dLorem ipsum dolor sit amet consectetur, adipisicing asd asdas
    //           dLorem ipsum dolor sit amet consectetur, adipisicing asd asdas
    //           dLorem ipsum dolor sit amet consectetur, adipisicing asd asdas
    //           dLorem ipsum dolor sit amet consectetur, adipisicing asd asdas d
    //         </p>
    //         <Link
    //           href={"https://github.com/SantiagoHayase/PI-videogames-Henry"}
    //         >
    //           <button className="px-8 py-2 mt-4">Code</button>
    //         </Link>
    //       </div>
    //     </div>
    //   </div>
    //   <Link href={"/#projects"}>
    //     <p>Back</p>
    //   </Link>
    // </div>
  );
};

export default videogamesapi;
